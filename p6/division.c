...
// Case 1
static void fpeHandler(int signal, siginfo_t *act, void *nullact) {
...
int main(int argc, const char *argv[]) {
    struct sigaction act, act2, nullact;
    int buffSize = 100;
    char buffer[100];
    int dividend;
    int divisor;
    ...
    act2.sa_flags = SA_ONSTACK | SA_RESTART | SA_SIGINFO;
    act.sa_sigaction = fpeHandler;
    act2.sa_sigaction = intHandler;
    ...
    if (sigaction(SIGINT, &act2, &nullact) < 0) {
        perror("Error handling signal");
        exit(0);
    }
